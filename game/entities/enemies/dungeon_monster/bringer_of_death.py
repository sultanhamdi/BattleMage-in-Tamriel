...
class BringerOfDeath(BaseEnemy):
    ...
    SPELL_COOLDOWN = 600  
    MELEE_RANGE = 70
    PHASE2_HP_THRESHOLD = 0.6  
    PHASE3_HP_THRESHOLD = 0.3  
    CAST_TRIGGER_HITS = 5 
    ...
    def __init__(self, x, y):
        ...
        self.lose_interest_range = 1000 
        
        # attack box
        self.attack_box_width = 140  
    ...
    def _update_ai(self):
        ...
        # PRIORITY, exit early to prevent other state changes
        if self.is_casting:
            ...
            # Check if cast animation reaches frame 5 
            # trigger spell transition
            if frame_idx >= 5:
                # Cast animation completed 
                # trigger spell damage and start spell visual
                self.cast_spell()
                spell_cd = self.SPELL_COOLDOWN
                if self.current_phase >= 2:
                    spell_cd = int(spell_cd * 0.7)  
                self.spell_cooldown = spell_cd
                
                # Switch to 'spell' state
                self.state = 'spell'
                self.ai_state = 'spell'
                self.is_casting = False  
        ...
        if distance <= self.SPELL_RANGE and self.spell_cooldown <= 0 and self.is_spell_ready:
            ...
            # SPELL CAST 
            # only if hit counter reached threshold
            self.ai_state = 'cast'
            self.state = 'cast'  
            self.is_casting = True
            self.is_spell_ready = False  
            self.hit_counter = 0  
            self.spell_target_player = True  
            ...
            # Check if player is stunned from our spell
            if hasattr(self.player_ref, 'is_stunned') and self.player_ref.is_stunned:
                # Player is suffering from our spell
                self.ai_state = self.STATE_IDLE
                self.physics.velocity_x = 0
